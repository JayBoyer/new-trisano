...
local_ce_staging_path="/mnt/tmp/trisano_deploy"
...
##    exit 0
    ...
    echo "Deploying"
    cap -v $cap_deploy_prefix deploy
...
##    ssh trisano_rails@$ssh_host "cd $remote_deployment_target_path/current; RAILS_ENV=uat bundle exec rake cache:warm[$live_url,100] --trace" &
