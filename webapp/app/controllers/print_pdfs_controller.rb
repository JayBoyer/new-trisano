...
=end
      ...
      if @@sthidro.fetchable?
           rows = @@sthidro.fetch_all
            if(rows != nil && rows.size > 0)
                ...
                if @idr_drug_o != '' && @idr_drug_o != nil
                  if @idr_drug_o.include? 'tb146_tb264'
                      if !idr_setOther
                        ...
                        @idro_var = 'tb264' + '_' + @idr_drug_o[-1,1]
                        @@hash_output[@idro_var] = @@smallx
      ...
      if @@sthidrs.fetchable?
           rows = @@sthidrs.fetch_all
            if(rows != nil && rows.size > 0)
              rows.each_with_index do |row, index|
                ...
                if @idrs_answer_text != '' && @idrs_answer_text != nil
                  ...
                  if index > 0 && (@@repeat_id != @idrs_repeat_id) && !@@setOther
                      evalRecord
                  end
      ...
      if @@sthfdrs.fetchable?
           rows = @@sthfdrs.fetch_all
            if(rows != nil && rows.size > 0)
              rows.each_with_index do |row, index|
                ...
                if @fdrs_answer_text != '' && @fdrs_answer_text != nil
                  ...
                  if index > 0 && (@@repeat_id != @fdrs_repeat_id) && !@@setOther
                      evalRecord
                  end
