...
When /^I fill in enough (.+) data to enable all core fields to show up in show mode$/ do |event_type|
  ...
  if event_type == "morbidity_event" || event_type == "contact_event" || event_type == "assessment_event"
    common_test_type = CommonTestType.find_by_common_name("Common Test Type")
    lab_name = PlaceEntity.by_name_and_place_type(PlacesSearchForm.new({:place_type => "L"})).first.place.name 
    hospital_name = PlaceEntity.by_name_and_place_type(PlacesSearchForm.new({:place_type => "H"})).first.place.name 
    add_lab_result(@browser, { :lab_name => lab_name, :test_type => common_test_type.common_name })
...
Given /^a common test type named "(.+)"$/ do |common_name|
